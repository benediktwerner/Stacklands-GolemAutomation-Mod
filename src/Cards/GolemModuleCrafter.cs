...
namespace GolemAutomation
{
    ...
    {
        public override bool CanInsert(Golem g) =>
            g.CraftingRecipe.Length == 0 && Recipe.Length > 0 && !g.HasSellingModule;
        ...
        public override bool CanHaveCard(CardData otherCard) =>
            !Card.IsAnimal(otherCard) && !otherCard.IsBuilding;
        ...
        public override void UpdateCard()
        {
            ...
            if (MyGameCard.Parent == null && recipe != null && recipe != Recipe)
            {
                MyGameCard.StartTimer(
                    1f,
                    new TimerAction(SetRecipe),
                    "Setting recipe",
                    GetActionId(nameof(SetRecipe))
                );
            }
            else
            {
        ...
        public string ComputeCurrentRecipe()
        {
            ...
            if (child == null)
                return null;
            if (
                child.CardData.IsBuilding
                || child.CardData.MyCardType == CardType.Humans
                || Card.IsAlive(child)
            )
                return null;
